...
**Источник чисел:** `DEFAULT_GAME_CONFIG` v12 (`packages/shared/src/config.ts`) — то, что сидится в БД и правится без редеплоя. **Источник формул:** `packages/shared/src/economy.ts`. Здесь — снимок дефолтов; истина в рантайме — live `GameConfig`.
...
|---|---|---|---|
| `join` | +75 | +50 | разовый |
| `premium` (у приглашённого Telegram Premium) | +150 | +150 | разовый |
| `passive` | отключён (`referralPassiveRate = 0`) | — | — |

- Гейт активности: бонусы начисляются только когда у приглашённого ≥ 1 завершённой coupon-сессии.
- Пассив **отключён**: `referralPassiveRate = 0`, новых строк `refSource='passive'` не создаётся (ветка `mintReferralPassive` сразу возвращает 0). Хард-кап `referralPassiveCap = 1_000_000` сохранён в схеме на случай возврата пассива через live `GameConfig`.
